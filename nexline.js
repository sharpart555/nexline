...
function nexline(param) {
  ...
  if (inputType === undefined)
    throw new Error('Invalid input. Input must be one of these: string, buffer, readable stream, file descriptor');
  ...
  for (const item of lineSeparatorStringList) {
    if (typeof item !== 'string' || item.length === 0)
      throw new Error('Invalid lineSeparator, lineSeparator must be string and must exceed one character');
  ...
  // Verify option
  if (reverse && inputType === INPUT_TYPE.STREAM)
    throw new Error('Stream cannot be read reversely. use other input like file descriptor');
